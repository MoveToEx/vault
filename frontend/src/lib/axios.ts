...
const startsWith = (str: string | undefined, prefix: string[]) =>
  !str ? false : prefix.some((it) => str.startsWith(it));

const AUTH_IGNORE = ["/auth/login", "/auth/register", "/auth/refresh"];
...
instance.interceptors.request.use(
  async (req) => {
    ...
    req.headers.Authorization =
      "Bearer " +
      JSON.parse(localStorage.getItem("vault-access-token") ?? '""');
